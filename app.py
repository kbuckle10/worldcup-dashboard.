...
function teamCard(t){{return `<div class='card team-card' data-team='${{esc(t.team)}}' data-status='${{esc(t.statusKey)}}'><div class='team-top'><div><div class='flag'>${{esc(t.flag)}}</div><div class='team-name'>${{esc(t.team)}} <span class='code'>${{esc(t.code)}}</span></div></div><span class='badge ${{t.statusKey==='eliminated'?'out':''}}'>${{esc(t.statusLabel)}}</span></div><div class='muted'>${{esc(t.standing)}}${{t.group?' • Group '+esc(t.group):''}}</div><div class='team-meta'><span class='tag'>${{esc(t.record)}} W-D-L</span><span class='tag'>GD ${{esc(t.gd)}}</span>${{formHtml(t)}}</div></div>`}}
