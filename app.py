...
st.markdown(
    ...
    <style>
      ...
      .wc-live-priority {border:1px solid rgba(239,68,68,.55); border-radius:20px; padding:12px; background:linear-gradient(120deg, rgba(239,68,68,.10), rgba(56,189,248,.05)); margin-bottom:16px;}
      .wc-section-kicker {color:#34d399; font-weight:900; letter-spacing:.09em; text-transform:uppercase; font-size:.76rem;}
    </style>
    """,
...
def live_minute(row: pd.Series) -> str:
    """Return the best available live clock.

    Prefer API elapsed/minute. If absent, estimate from kickoff so a live game
    still shows a match clock instead of only LIVE.
    """
    if row.get("status") != "Live":
        return ""
    elapsed = clean_text(row.get("elapsed"))
    if elapsed and elapsed.lower() not in {"live", "in_play", "none", "null"}:
        return elapsed if elapsed.endswith("'") else f"{elapsed}'"
    dt = row.get("date_time")
    if isinstance(dt, datetime):
        try:
            mins = int((datetime.now() - dt.replace(tzinfo=None)).total_seconds() // 60)
            if 0 <= mins <= 130:
                if mins <= 45:
                    return f"{mins}'"
                if mins <= 60:
...
def render_match_centre(row: pd.Series) -> None:
    """Interactive Match Centre for a selected fixture."""
    home = clean_text(row.get("home_team", "TBD"), "TBD")
    away = clean_text(row.get("away_team", "TBD"), "TBD")
    score = scoreline_label(row)
    minute = live_minute(row) or ("FT" if row.get("status") == "Finished" else clean_text(row.get("kickoff", "TBD")))

    st.markdown(f"### {team_flag(home)} {home} {score} {team_flag(away)} {away}")
    st.caption(f"{row.get('stage_label','')} • {row.get('status','')} • {minute} • {row.get('kickoff','TBD')}")

    c1, c2, c3 = st.columns(3)
    c1.metric("Status", row.get("status", "Scheduled"))
    c2.metric("Clock", minute)
    c3.metric("Venue", clean_text(row.get("venue"), "TBD"))

    hs = 0 if pd.isna(row.get("home_score")) else int(row.get("home_score"))
    aw = 0 if pd.isna(row.get("away_score")) else int(row.get("away_score"))
    st.markdown("#### Match breakdown")
    st.dataframe(
        pd.DataFrame([
            {"Metric": "Goals", home: hs, away: aw},
            {"Metric": "Stage", home: row.get("stage_label", ""), away: row.get("stage_label", "")},
            {"Metric": "Winner", home: clean_text(row.get("winner"), "—"), away: clean_text(row.get("winner"), "—")},
        ]),
    ...
    progress = timeline_percent(row) if row.get("status") == "Live" else (100 if row.get("status") == "Finished" else 0)
    st.progress(progress / 100, text=f"{minute} — {progress}% of regulation time")

    home_scorers = clean_text(row.get("home_scorers"))
    away_scorers = clean_text(row.get("away_scorers"))
    if home_scorers or away_scorers:
        st.markdown("#### Scorers")
        if home_scorers:
            st.write(f"**{home}:** {home_scorers}")
        if away_scorers:
            st.write(f"**{away}:** {away_scorers}")
    else:
        st.info("Detailed events appear here when the selected data source provides scorer/event fields.")

    with st.expander("Raw source data"):
        st.json(row.get("raw", {}))


def open_match_centre_button(row: pd.Series, key_prefix: str) -> None:
    label = f"Open Match Centre — {clean_text(row.get('home_team'))} vs {clean_text(row.get('away_team'))}"
    mid = clean_text(row.get("match_id")) or str(abs(hash(str(row.to_dict()))) % 100000)
    key = f"{key_prefix}_{mid}"
    if hasattr(st, "dialog"):
        @st.dialog("Match Centre", width="large")
        def _dialog():
...
def render_matches_tab(matches_df: pd.DataFrame) -> None:
    st.header("Upcoming & Live")
    st.markdown("Live games stay pinned at the top. Click a match to open the Match Centre with clock, timeline, source stats and raw event details.")
    live_now = matches_df[matches_df["status"] == "Live"].sort_values("date_time", na_position="last") if not matches_df.empty else pd.DataFrame()
    if not live_now.empty:
        st.markdown('<div class="wc-section-kicker">🔴 Live now</div>', unsafe_allow_html=True)
        for i, (_, row) in enumerate(live_now.iterrows()):
            st.markdown('<div class="wc-live-priority">', unsafe_allow_html=True)
            render_live_score_card(row)
            open_match_centre_button(row, f"live_top_{i}")
            st.markdown('</div>', unsafe_allow_html=True)
    else:
        st.info("No match is marked live by the current data source. Upcoming fixtures are listed below.")

    st.markdown("### All fixtures and results")
    ...
    for idx, (_, row) in enumerate(filtered.head(20).iterrows()):
        ...
        open_match_centre_button(row, f"match_card_{idx}")
    if len(filtered) > 20:
...
def main() -> None:
    ...
    # Global tournament banner stays above the navigation tabs.
    ...
    tab_dashboard, tab_matches, tab_standings, tab_knockout, tab_teams, tab_insights, tab_guide, tab_deploy = st.tabs(
        ["Overview", "Upcoming & Live", "Groups & Standings", "Knockout Bracket", "Teams", "Stats & Insights", "Learn the Basics", "Deploy"]
    )
    with tab_dashboard:
